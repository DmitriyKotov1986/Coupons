...
{
  //  qDebug() << "Connect: " << Url;
  //  qDebug() << "Total send byte: " << data.size();
    ...
    if (resp == nullptr) {
        emit SendLogMsg(TCouponsUpdater::CODE_ERROR, "Send HTTP request fail.");
        emit ErrorOccurred();
        return false;
    }
...
void THTTPQuery::onReplyFinished(QNetworkReply *resp)
{
   // qDebug() << "HTTP Finished";
    if (resp->isOpen()) {
        ...
        emit SendLogMsg(TCouponsUpdater::CODE_ERROR, "Response from HTTP server not received. Error: " + resp->errorString());
        emit ErrorOccurred();
    }
    resp->deleteLater();
}
...
void THTTPQuery::onErrorOccurred(QNetworkReply::NetworkError ErrCode)
{
    ...
    emit SendLogMsg(TCouponsUpdater::CODE_ERROR, "HTTP request fail. Msg: " + NetworkError2Str(ErrCode));
    resp->deleteLater();
    emit ErrorOccurred();
}
